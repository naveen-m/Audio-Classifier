...
* McGill Univarsity
* 
* Modified by - Naveen, Usha & Pooja (for assignment 4)
*/
...
// the required package to process WEKA and xmlReader
import dataModelWeka.XmlReader;
...
public class FeatureSelectorPanel extends JPanel implements ActionListener {
	...
	*/
	...
	// Added this button which links to WEKA processing for the assignment 
	private JButton test_features_button;
	...
	*/
	...
	// String used to get processed results, and display.
	...
	*/
	public FeatureSelectorPanel(OuterFrame outer_frame, Controller c) {
		...
		test_features_button = new JButton("Test data results");
		test_features_button.addActionListener(this); 
		control_panel.add(test_features_button);
	...
	*/
	public void actionPerformed(ActionEvent event) {
		...
		// Action performed added for the test button
		else if(event.getSource().equals(test_features_button))
		{
			/* Calls the xmlreader to process the extracted feature ouptut
			* of jAudio, writes to weka.txt and also passes the retrived testdata
			* filenames to WEKA to process and send back the final test data results
			*  string to display .
			*/
			String[] testdataNames = xmlreader.read();
			WekaTest weka = new WekaTest();
			try {
				results = weka.getResults(testdataNames);
			} catch (Exception e) {
			...
			null,
			results,
			"TEST DATA RESULTS", JOptionPane.INFORMATION_MESSAGE);
